...
from .embedded import (
    ...
    _create_embedded_client, _export_port_health_grace_timeout, _load_simple_env,
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def flush_pending(self, timeout: float | None = None) -> bool:
        """Wait for queued retains and tracked async operations to become durable."""
        if self._writer_thread is None:
            return True
        if timeout is None:
            while self._retain_queue.unfinished_tasks:
                time.sleep(0.05)
            return self._wait_for_server_retain_ops(lambda: False, 0.0)
        timeout = max(0.0, float(timeout))
        if timeout == 0:
            return not self._retain_queue.unfinished_tasks and not self._pending_retain_ops
