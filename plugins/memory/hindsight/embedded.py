...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    try:
        # The top-level ``hindsight`` package eagerly imports its API server and
        # FastMCP. Hermes uses MCP 2.x while that server currently targets MCP
        # <2, so embedded mode talks to the daemon through the client and daemon
        # manager modules without importing the in-process server.
        for module in ("hindsight_client", "hindsight_embed.daemon_embed_manager", "sentence_transformers"):
...
class _IsolatedEmbeddedHindsight:
    """Embedded client that keeps Hindsight's MCP server out of Hermes' process."""
