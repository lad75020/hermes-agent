...
- `apple` is an explicit macOS 26+ on-device provider backed by `SpeechAnalyzer` and `SpeechTranscriber`. `stt.apple.download_assets` defaults to `false`; set it to `true` only if you authorize Apple's selected-locale asset download. Its language precedence is hook override → `stt.apple.language` → `stt.language` → `HERMES_LOCAL_STT_LANGUAGE` → system locale. The system-locale fallback is not speech-language auto-detection.
