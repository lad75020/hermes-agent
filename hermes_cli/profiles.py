...
def get_profile_dir(name: str) -> Path:
    ...
    if existing := _find_profile_dir_case_insensitive(canon):
        return existing
    return _get_profiles_root() / canon


def _find_profile_dir_case_insensitive(canon: str) -> Optional[Path]:
    """Find an existing profile directory for *canon*, including legacy mixed-case names."""
    profiles_root = _get_profiles_root()
    if not profiles_root.is_dir():
        return None
    try:
        entries = [entry for entry in profiles_root.iterdir() if entry.is_dir()]
        return next((entry for entry in entries if entry.name == canon), None) or next(
            (entry for entry in entries if entry.name.casefold() == canon.casefold()), None)
    except OSError:
        return None
...
def _iter_named_profile_dirs(*, live_only: bool = True) -> List[Path]:
    ...
    for entry in sorted(profiles_root.iterdir()):
        ...
        try:
            ...
            continue
        if canon == "default" or not _PROFILE_ID_RE.match(canon):
            continue
        if live_only and named_profile_is_deleted(entry):
            continue
        # Prefer an exact canonical directory if both it and a legacy mixed-case spelling exist.
        if canon not in profiles or entry.name == canon:
...
def list_profiles() -> List[ProfileInfo]:
    ...
    if named:
        ...
        for entry in named:
            ...
            profiles.append(_profile_info(name, entry, is_default=False, alias_name=alias_map.get(name)))
    return profiles
