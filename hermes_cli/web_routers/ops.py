...
from hermes_cli.web_models import (
    ApprovalResolve, BackupRequest, CredentialPoolAdd, HookCreate, HookDelete, ImportRequest, MemoryProviderSelect,
...
# --- Pending approvals: dashboard/native clients resolve the same FIFO queues
# used by messaging platforms without exposing their private threading.Event.


@router.get("/api/approvals")
async def get_approvals():
    """List pending gateway approval requests."""
    try:
...
def _normalize_approval_choice(choice: str) -> str:
    ...
    normalized = {
        "approve": "once", "allow": "once", "yes": "once", "no": "deny",
...
def _resolve_dashboard_approval(session_key: str, choice: str, *, resolve_all: bool = False):
    ...
    try:
        ...
        resolved = resolve_gateway_approval(
            session_key, normalized_choice, resolve_all=bool(resolve_all))
    ...
    return {
        "ok": True, "session_key": session_key, "choice": normalized_choice,
        "resolve_all": bool(resolve_all), "resolved": resolved,
    }
...
async def resolve_approval(body: ApprovalResolve):
    return _resolve_dashboard_approval(
        body.session_key or "", body.choice, resolve_all=body.resolve_all)


@router.post("/api/approvals/{session_key}/resolve")
async def resolve_approval_for_session(session_key: str, body: ApprovalResolve):
    return _resolve_dashboard_approval(
        session_key, body.choice, resolve_all=body.resolve_all)
