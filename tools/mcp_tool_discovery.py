...
async def _connect_server(name: str, config: dict):
    """Create the configured MCP/OpenAPI task and return once ready (tear down with ``server.shutdown()``
    ...
    server = OpenAPIServerTask(name) if is_openapi_server_config(config) else _core.MCPServerTask(name)
