...
def test_model_options_explicit_profile_ignores_stale_session_agent(monkeypatch):
    stale_agent = object()
    server._sessions["stale-session"] = _session(agent=stale_agent)
    picker_context = Mock(return_value=object())
    monkeypatch.setattr(server, "_model_picker_context", picker_context)
    monkeypatch.setattr(
        "hermes_cli.inventory.build_model_options_payload",
        lambda *_args, **_kwargs: {"providers": [], "model": "", "provider": ""},
    )

    try:
        resp = server._methods["model.options"](
            103,
            {"session_id": "stale-session", "profile": "default"},
        )
    finally:
        server._sessions.pop("stale-session", None)

    assert "result" in resp, resp
    picker_context.assert_called_once_with(None)
